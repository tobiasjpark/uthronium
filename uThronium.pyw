    ...
    def __init__(self, x):
        if x == True:
            try:
                ...
                self.m = ""
                self.n = 0
                self.x = self.window.create_text(10, 40, text=self.m, font=('Belwe-Condensed', 20), fill='black', anchor=W)
                ...
                if len(bac) > 1:
                    if bac == 'BackSpace':
                        if self.n > 0:
                            ...
                            self.n = self.n - 1
                            t = '*' * self.n
                            self.window.itemconfig(self.x, text=t, font=('Belwe-Condensed', 20), fill='black', anchor=W)
                    elif bac == 'space':
                        self.m = self.m + ' '
                        self.n = self.n + 1
                        t = '*' * self.n
...
class wargame1():
    def __init__(self):
        try:
            ...
            turtle.write("Loading Game. Please Wait!", align=CENTER, font=('Belwe-Condensed', 36))
            setup = turtle.Turtle()
            setup.up()
            setup.speed(0)
            setup.forward(320)
                ...
                if end == False:
                    ...
                    for y in turtlelist:
                       ...
                       if touching(self.ship.pos(), n.pos(), force) or touching(self.ship.pos(), o.pos(), force) or touching(self.ship.pos(), p.pos(), force) or touching(self.ship.pos(), q.pos(), force) or touching(self.ship.pos(), r.pos(), force) or touching(self.ship.pos(), s.pos(), force) or touching(self.ship.pos(), a.pos(), force) or touching(self.ship.pos(), b.pos(), force) or touching(self.ship.pos(), c.pos(), force) or touching(self.ship.pos(), d.pos(), force) or touching(self.ship.pos(), e.pos(), force) or touching(self.ship.pos(), f.pos(), force) or touching(self.ship.pos(), g.pos(), force) or touching(self.ship.pos(), h.pos(), force) or touching(self.ship.pos(), i.pos(), force) or touching(self.ship.pos(), j.pos(), force) or touching(self.ship.pos(), k.pos(), force) or touching(self.ship.pos(), l.pos(), force) or touching(self.ship.pos(), m.pos(), force): 
                            ...
                            t = time() - t
                            t = int(t)
                            turtle.write("Game Over! Your score is %s" % t, align=CENTER, font=('Belwe-Condensed', 36))
                            return None
                if falling(a):
    ...
    try:
        ...
        if details["user"] == None or abcd == True:
            ...
            if x == 63:
                ...
                speak('''You have just won a war with Mexico! You are entitled to some money. However, if you ask for too much money, Mexico might get angry and go back to war. How much would you like to take from Mexico?''')
...
How much would you like to take from Mexico?''')
                ...
                speak('''It is time to collect monthly taxes in one of your cities. The population of this city is 25, 000. (Not to be confused with the population of your country). How much would you like to charge each citizen?''')
                y = simpledialog.askfloat("What's Up, Casey?", '''
...
def drawpas():
    try:
        ...
        canvas.create_text(400, 50, text="Passwords", font=('Belwe-Condensed', 72), fill='black')
        ...
        canvas.create_text(310, 310, text="Price: $5,000,000", font=('Belwe-Condensed', 12), fill='black')
        canvas.create_text(510, 310, text="Price: $5,200,000", font=('Belwe-Condensed', 12), fill='black')
    except Exception:
        er(format_exc())
def df1():
...
def drawframe():
    try:
        ...
        drawcan()
        canvas.create_rectangle(0, 0, 800, 600, fill="#F1EEE5")
        canvas.create_text(400, 50, text="Picture Frames", font=('Belwe-Condensed', 72), fill='black')
        bac = Button(root, text="<- Back to Store", command=drawstore)
        bac.place(relx=.050, rely=.850)
        ...
        canvas.create_text(160, 300, text="Frame 1", fill='black', font=('Belwe-Condensed', 16))
        canvas.create_text(400, 300, text="Frame 2", fill='black', font=('Belwe-Condensed', 16))
        canvas.create_text(640, 300, text="Frame 3", fill='black', font=('Belwe-Condensed', 16))
        ...
        canvas.create_text(160, 350, text="Price: $150", font=('Belwe-Condensed', 12), fill='black')
        c1 = Button(root, text="Details", command=df3, padx=50)
        c1.place(relx=.71, rely=.52)
        canvas.create_text(400, 350, text=" Price: $200", font=('Belwe-Condensed', 12), fill='black')
        canvas.create_text(640, 350, text="Price: $300", font=('Belwe-Condensed', 12), fill='black')
    except Exception:
        er(format_exc())
...
def drawother():
    try:
        ...
        drawcan()
        canvas.create_rectangle(0, 0, 800, 600, fill="#F1EEE5")
        canvas.create_text(400, 50, text="Other", font=('Belwe-Condensed', 72), fill='black')
        bac = Button(root, text="<- Back to Store", command=drawstore)
        bac.place(relx=.050, rely=.850)
        a = Button(root, text="Buy Another Life", command=newlife)
        a.place(relx=.1, rely=.4)
        b = Button(root, image=personprev, command=visual)
        canvas.create_text(275, 170, text="Visual Person Casey", fill='black', font=('Belwe-Condensed', 16))
        ...
        canvas.create_text(135, 320, text="Price: $5 Billion", font=('Belwe-Condensed', 12), fill='black')
        canvas.create_text(280, 400, text="Price: $10 Thousand", font=('Belwe-Condensed', 12), fill='black')
        canvas.create_text(455, 320, text="Price: Your Choice", font=('Belwe-Condensed', 12), fill='black')
        canvas.create_text(650, 320, text="Price: $5,000 Initial Cost", font=('Belwe-Condensed', 12), fill='black')
    except Exception:
        er(format_exc())
def stock():
    try:
...
def drawstore2(x):
    try:
        if details["user"] == None and x != False or debug["status"] == 'color':
            ...
            canvas.create_text(400, 50, text="Store", font=('Belwe-Condensed', 72), fill='black')
            a = Button(root, image=trprev, command=drawtroom)
            a.place(relx=.050, rely=.300)
            canvas.create_text(120, 320, text="Throne Rooms", font=('Belwe-Condensed', 16), fill='black')
            p = Button(root, image=puz, command=drawpas)
            p.place(relx=.300, rely=.270)
            canvas.create_text(320, 330, text="Passwords", font=('Belwe-Condensed', 16), fill='black')
            frame = Button(root, image=fprev, command=drawframe)
            frame.place(relx=.520, rely=.330)
            canvas.create_text(520, 330, text="Picture Frames", font=('Belwe-Condensed', 16), fill='black')
            oth = Button(root, image=other, command=drawother)
            oth.place(relx=.85, rely=.250)
            canvas.create_text(720, 330, text="Other", font=('Belwe-Condensed', 16), fill='black')
...
def drawdesk():
    try:
        ...
        b.place(relx=.050, rely=.850)
        canvas.create_text(400, 50, text="Return An Item", font=('Belwe-Condensed', 72), fill='black')
        canvas.create_text(390, 150, text="Choose an item to return.", font=('Belwe-Condensed', 30), fill='black')
...
def drawstoreroom():
    try:
        ...
        drawcan()
        canvas.create_rectangle(0, 0, 800, 600, fill="#F1EEE5")
        canvas.create_text(400, 50, text="Throne Rooms - Storeroom", font=('Belwe-Condensed', 48), fill='black')
        canvas.create_text(400, 300, text="A shaded button means that that room has not be bought, or is being used.", font=('Belwe-Condensed', 19), fill='black')
        ...
        canvas.create_text(120, 260, text="Room 0", font=('Belwe-Condensed', 16), fill='black')
        b = Button(root, image=fp1, command=drawtr01)
        b.place(relx=.270, rely=.200)
        canvas.create_text(300, 260, text="Room 1", font=('Belwe-Condensed', 16), fill='black')
        c = Button(root, image=fp2, command=drawtr02)
        c.place(relx=.49, rely=.2)
        canvas.create_text(480, 260, text="Room 2", font=('Belwe-Condensed', 16), fill='black')
        d = Button(root, image=fp3, command=drawtr03)
        d.place(relx=.71, rely=.2)
        canvas.create_text(650, 260, text="Room 3", font=('Belwe-Condensed', 16), fill='black')
        e = Button(root, image=fp4, command=drawtr04)
        e.place(relx=.27, rely=.575)
        canvas.create_text(300, 490, text="Room 4", font=('Belwe-Condensed', 16), fill='black')
        f = Button(root, image=fp5, command=drawtr05)
        f.place(relx=.49, rely=.575)
        canvas.create_text(475, 490, text="Room 5", font=('Belwe-Condensed', 16), fill='black')
        g = Button(root, image=fp6, command=drawtr06)
        g.place(relx=.71, rely=.575)
        canvas.create_text(650, 490, text="Room 6", font=('Belwe-Condensed', 16), fill='black')
        y = 0
        for x in range(0, 7):
...
def drawtroom():
    try:
        ...
        canvas.create_text(400, 50, text='''Throne Rooms''', font=('Belwe-Condensed', 72), fill='black')
        ...
        canvas.create_text(300, 320, text="Price: $100,000", font=('Belwe-Condensed', 12), fill='black')
        ...
        canvas.create_text(480, 320, text="Price: $300,000", font=('Belwe-Condensed', 12), fill='black')
        d = Button(root, image=fp3, command=drawtr3)
        d.place(relx=.71, rely=.2)
        d1 = Button(root, text="Details", command=drs4)
        d1.place(relx=.77, rely=.45)
        canvas.create_text(650, 260, text="Room 3", font=('Belwe-Condensed', 16), fill='black')
        canvas.create_text(650, 320, text="Price: $500,000", font=('Belwe-Condensed', 12), fill='black')
        e = Button(root, image=fp4, command=drawtr4)
        e.place(relx=.27, rely=.575)
        e1 = Button(root, text="Details", command=drs5)
        e1.place(relx=.335, rely=.84)
        canvas.create_text(300, 490, text="Room 4", font=('Belwe-Condensed', 16), fill='black')
        canvas.create_text(300, 550, text="Price: $200,000", font=('Belwe-Condensed', 12), fill='black')
        ...
        canvas.create_text(475, 490, text="Room 5", font=('Belwe-Condensed', 16), fill='black')
        canvas.create_text(475, 550, text="Price: $1,000,000", font=('Belwe-Condensed', 12), fill='black')
        g = Button(root, image=fp6, command=drawtr6)
        g.place(relx=.71, rely=.575)
        g1 = Button(root, text="Details", command=drs7)
        g1.place(relx=.77, rely=.841)
        canvas.create_text(650, 490, text="Room 6", font=('Belwe-Condensed', 16), fill='black')
        canvas.create_text(650, 550, text="Price: $1,500,000", font=('Belwe-Condensed', 12), fill='black')
    except Exception:
        er(format_exc())
def drawacc():
    warm()
...
Kingdom''', font=('Belwe-Condensed', 15), anchor=W, command=exit_save)
            lo.place(relx=.47, rely=.84)
            store = Button(canvas, text="Store", font=('Belwe-Condensed', 15), anchor=W, command=drawstore)
            store.place(relx=.581, rely=.84)
            i = Button(canvas, text="Instructions", font=('Belwe-Condensed', 15), anchor=W, command=instruct)
            i.place(relx=.204, rely=.84)
            r = Button(canvas, text='''Resign
(Delete Account)''', font=('Belwe-Condensed', 15), anchor=W, command=resig)
...
Minigame''', font=('Belwe-Condensed', 15), anchor=W, command=drawacc)
            accb.place(relx=.655, rely=.84)

            up = Button(canvas, text='''Update Kingdom
Status
(Ticker Tape)''', font=('Belwe-Condensed', 15), anchor=W, command=update)
            up.place(relx=.775, rely=.84)
            wum = Button(canvas, text='''What's Up,
Casey?''', font=('Belwe-Condensed', 15), command=randmurph)#, anchor=W)
            ...
            canvas.create_text(390, 460, text=details["News"], font=('Belwe-Condensed', 12))
            
            dothing = Button(canvas, text='''Casey's Voice
On/Off''', font=('Belwe-Condensed', 15), anchor=W, command=toggle)
            ...
            canvas.create_text(680, 50, text="Money:", font=('Belwe-Condensed', 18))
            temp["money"] = canvas.create_text(680, 70, text=details["money"], font=('Belwe-Condensed', 14))
            canvas.create_text(680, 165, text="Lives:", font=('Belwe-Condensed', 18))
            canvas.create_text(680, 185, text=details["lives"], font=('Belwe-Condensed', 14))
            if details["person"] == True:
...
(New Users Click Here)''', command=create_king)
        ...
        canvas.create_text(400, 150, text="Welcome to uThronium!", font=('Belwe-Condensed', 48), fill='white')
        canvas.create_text(400, 350, text="Press a button to begin your adventure.", font=('Belwe-Condensed', 20), fill='white')
...
try:
    ...
    def firstdrawcan():
        ...
        canvas.pack()
        canvas.create_text(400, 300, text="uhoh", font=('Belwe-Condensed', 72))
    def newinhere():
...
''')
    ...
    if setu == True:
        ...
        if uname()[0] != 'Linux':
            ...
            other = personprev
            c_1 = PhotoImage(file=str(abc) + str('cp1.gif'))
            c_2 = PhotoImage(file=str(abc) + str('cp2.gif'))
            c_3 = PhotoImage(file=str(abc) + str('cp3.gif'))
            c_4 = PhotoImage(file=str(abc) + str('cp4.gif'))
            c_5 = PhotoImage(file=str(abc) + str('cp5.gif'))
            c_6 = PhotoImage(file=str(abc) + str('cp6.gif'))
            c_7 = PhotoImage(file=str(abc) + str('cp7.gif'))
            c_8 = PhotoImage(file=str(abc) + str('cp8.gif'))
            c_9 = PhotoImage(file=str(abc) + str('cp9.gif'))
        else:
            debug["bit"] = None
            abc = ""
